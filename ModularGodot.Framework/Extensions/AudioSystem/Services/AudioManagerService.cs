...
using Extensions.AudioSystem.Repositories.Abstractions;
using Extensions.AudioSystem.Services.Abstractions;

namespace Extensions.AudioSystem.Services;
...
public class AudioManagerService : BaseService, IAudioManagerService
{
    ...
    public void PreloadAudio(string path, AudioEnums.AudioType type)
    {
        ...
        if (_resourceLoader.LoadResource(path) is AudioStream audioResource)
        {
            ...
            // 触发扩展点
            var extensionPoints = ExtensionManager.Instance.GetExtensionPoints<IAudioSystemExtensionPoint>();
            foreach (var point in extensionPoints)
            {
                point.OnAudioPlayed(path, 1.0f);
            }
        }
        else
        {
    ...
    public void PlayMusic(string path, float fadeDuration = 1.0f, bool loop = true)
    {
        ...
        if (_audioManagerRepo.CurrentMusic != null)
        {
            ...
            foreach (var point in extensionPoints)
            {
                point.OnAudioPlayed(path, _audioManagerRepo.Volumes[AudioEnums.AudioType.Music]);
            }
        }
        else
        {
    ...
    public AudioStreamPlayer PlaySound(string path, float volume = 1.0f)
    {
        ...
        foreach (var point in extensionPoints)
        {
            point.OnAudioPlayed(path, volume);
        }
    ...
    public void SetVolume(AudioEnums.AudioType type, float volume)
    {
        ...
        foreach (var point in extensionPoints)
        {
            point.OnVolumeChanged(type, volume);
        }
    }
